...
Thanya Car Spare Part System is a dedicated web and API-driven platform built to handle the transactional and operational needs of car spare parts distributors. The system acts as a bridges connecting stock managers, parts suppliers, and auto repair shops (or individual customers) who need to find, verify, and buy exact automotive components.

## What the Project Is

At its core, Thanya is an e-commerce catalog combined with a multi-warehouse inventory tracker and a vehicle compatibility engine. It exposes a RESTful API powered by ASP.NET Core and a lightweight, responsive Web UI. It organizes thousands of individual parts, matching them to specific vehicle profiles (brands, model revisions, production periods) to eliminate the risk of mismatching parts.
...
Thanya was built to enforce compatibility checks directly at the database and API levels, helping customers locate the correct components without relying on manual catalog reading.
...
Using the Thanya portal, the mechanic selects:
- Brand: `BMW`
- Model: `3 Series (F30)`
